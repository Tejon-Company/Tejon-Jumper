...
class HUD:
    ...
    _coin_icon = None
    _energy_icon = None
    _bear_health_icon = None

    @classmethod
    def initialize(cls):
        cls._resolution_settings = ResolutionSettings()
        cls._font_size = int(cls._resolution_settings.tile_size * 0.7)
        cls._font = ResourceManager.load_font("Beta54.ttf", cls._font_size)

        cls._player_icon = ResourceManager.load_image("badger_icon.png")
    ...
    def _setup_health_icon(cls):
        ...
        health_icon = health_icon_image.subsurface(
            (0, 0, cls._resolution_settings.tile_size, cls._resolution_settings.tile_size)
        )

        cls._health_icon = health_icon
    ...
    def _setup_coin_icon(cls):
        ...
        coin_icon = coin_icon_image.subsurface(
            (
                ((cls._resolution_settings.tile_size * 3) + 2),
                0,
                cls._resolution_settings.tile_size,
                cls._resolution_settings.tile_size,
            )
        )

        cls._coin_icon = coin_icon
    ...
    def _setup_energy_icon(cls):
        ...
        energy_icon = energy_icon_image.subsurface(
            (
                ((cls._resolution_settings.tile_size * 2) + 1),
                0,
                cls._resolution_settings.tile_size,
                cls._resolution_settings.tile_size,
            )
    ...
    def _setup_bear_health_icon(cls):
        ...
        cls._bear_health_icon = bear_health_icon
    ...
    def _draw_icon_with_counter(cls, display_surface, icon, count: int, x: int, y: int):
        ...
        text_surface = cls._font.render(count_text, True, "white")
        display_surface.blit(text_surface, (x + cls._resolution_settings.tile_size, y))

    @classmethod
    def _draw_health_icons(
        cls, display_surface, health_points: int, icon, start_x: int, start_y: int
    ):
        heart_size = cls._resolution_settings.tile_size
        spacing = cls._resolution_settings.tile_size // 5
    ...
    def _draw_hearts(cls, display_surface, remaining_health_points):
        cls._draw_health_icons(
            display_surface, remaining_health_points, cls._health_icon, 10, 10
        )

    @classmethod
    def _draw_energy_bar(cls, display_surface, energy: float):
        bar_width = cls._resolution_settings.window_width * 0.15
        bar_height = cls._resolution_settings.tile_size // 2
        icon_width = cls._resolution_settings.tile_size
        icon_spacing = cls._resolution_settings.tile_size // 4
        start_x = cls._resolution_settings.tile_size // 5
        start_y = cls._resolution_settings.tile_size * 1.6

        bar_x = start_x - cls._resolution_settings.tile_size + icon_width + icon_spacing
        ...
        icon_x = start_x
        icon_y = start_y + (bar_height - cls._resolution_settings.tile_size) // 2
        display_surface.blit(cls._energy_icon, (icon_x, icon_y))
    ...
    def _draw_player_lifes_counter(cls, display_surface, remaining_lives: int):
        cls._draw_icon_with_counter(
            ...
            cls._player_icon,
            remaining_lives,
            cls._resolution_settings.tile_size // 5,
            cls._resolution_settings.tile_size * 2.5,
        )
    ...
    def _draw_coins_counter(cls, display_surface, coins: int):
        coin_icon_x = (
            ...
            - cls._coin_icon.get_width()
            - cls._resolution_settings.tile_size
        )
        cls._draw_icon_with_counter(
            display_surface, cls._coin_icon, coins, coin_icon_x, 10
        )

    @classmethod
    def _draw_bear_hearts(cls, display_surface, bear_healt_points: int):
        start_x = (
            cls._resolution_settings.window_width // 2
            - cls._resolution_settings.tile_size * 2
        )
        cls._draw_health_icons(
            display_surface, bear_healt_points, cls._bear_health_icon, start_x, 10
        )

...
class SettingsMenu(Scene):
    ...
    def draw(self, display_surface):
        ...
        self._draw_volume_bar(
            display_surface, self.effects_volume_bar, self.effects_volume, "Sound Effects", font)

        self._draw_button(display_surface, self.difficulty_btn,
                          f"Game mode: {self.difficulty_levels[self.current_difficulty]}", font)
        self._draw_button(display_surface, self.resolution_btn,
                          f"Resolution: {self.resolutions[self.current_resolution][0]}x{self.resolutions[self.current_resolution][1]}", font)
